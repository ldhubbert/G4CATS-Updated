...
void H100MeVsmearM.C()
{
	...
	//The more we want to smear the results by, the wider the Gaussian curve will be, attributing more error to the histogram results.i
	...
	//Create histogram
	TH1F *h1 = new TH1F("Histogram 1 Statistics", "", 300, 180, 210);
	...
	h1->SetTitle("0.75% Gaussian Smear on 200MeV Beam");
	h1->Draw();

	TLine *line = new TLine(200, 0, 200, 1925);
	line->Draw();
	...
	TH1F *h2 = new TH1F("Histogram 2 Statistics", "", 300, 180, 210);
	while (r2.Next())
	{
	...
	int binB = 0;
	//This searches for the bin in the range of BinWithMostCounts and bin 300, which is the last bin
	h1->GetBinWithContent(HalfMaxYValue, binB, BinWithMostCounts, 300, 0);
	//The if condition pertains if no bin was found with content of HalfMaxYValuei
	if (binB == 0)
	{
	...
	TPaveLabel *a = new TPaveLabel(182,1550,188,1800, FWHM_string);
	a->Draw();
}
